---
title: "Predicting log-transformed WoS citations with machine learning models: Linear regression"
author: "Aaron Plex"
...
WoSCGcont<-WoSCG_dataset[,c(1,10:22)]%>%
  mutate_all(as.numeric)%>%
  filter(AR_Year!=2021)%>%
  mutate(WoScitation=log10(WoScitation+1))
  
WoSCG_untidy<-gather(WoSCGcont, key = "Variable",
                     value = "Value", -WoScitation)

ggplot(WoSCG_untidy, aes(Value, WoScitation))+
...
WoSimp<-impute(as.data.frame(WoSCGcont),
               classes = list(numeric = imputeMethod))

#Defining our task and learner
WoStask<-makeRegrTask(data = WoSimp$data, target = "WoScitation")
...
WosSelFeat<-WoSimp$data[,c("WoScitation",selFeats$x)]
WosSelFeatTask<-makeRegrTask(data = WosSelFeat, target = "WoScitation")
...
WoSTaskWithNAs<-makeRegrTask(data = WoSCGcont, target = "WoScitation")
...
## Women team size evolution
```{r,dpi=200}
ggplot(data = WoSCGcont,aes(x=female, group=as.character(AR_Year)))+
  geom_density(aes(color=as.character(AR_Year), fill = as.character(AR_Year)), alpha = 0.4)+
  scale_color_viridis(discrete = TRUE, option = "viridis")+
  scale_fill_viridis(discrete = TRUE, option = "viridis")+
  scale_x_log10()+
  labs(x="Women team size", y="Density",
       fill="Year of publication",
       color="Year of publication")+
  theme(legend.position = c(0.8,0.71))
```

## Mean team size evolution
```{r,dpi=200}
ggplot(data = WoSCGcont,aes(x=team_size-female, group=as.character(AR_Year)))+
  geom_density(aes(color=as.character(AR_Year), fill = as.character(AR_Year)), alpha = 0.4)+
  scale_color_viridis(discrete = TRUE, option = "viridis")+
  scale_fill_viridis(discrete = TRUE, option = "viridis")+
  scale_x_log10()+
  labs(x="Men team size", y="Density",
       fill="Year of publication",
       color="Year of publication")+
  theme(legend.position = c(0.8,0.71))
```
...
## Team size evolution
```{r,dpi=200}
ggplot(data = WoSCGcont,aes(x=team_size, group=as.character(AR_Year)))+
  geom_density(aes(color=as.character(AR_Year), fill = as.character(AR_Year)), alpha = 0.4)+
  scale_color_viridis(discrete = TRUE, option = "viridis")+
  scale_fill_viridis(discrete = TRUE, option = "viridis")+
  scale_x_log10()+
  labs(x="Team size", y="Density",
       fill="Year of publication",
       color="Year of publication")+
  theme(legend.position = c(0.8,0.71))
```
...
team_size_distribution<-merge(authorlist[,c(1,2)], journalist[,c(1,2)])%>%
  filter(AR_Year!=2021)%>%
  group_by(team_size, AR_Year)%>%
  summarise(count=length(team_size))

ggplot(team_size_distribution, aes(x=team_size,y=count))+
  geom_smooth(se = FALSE, color = "black")+
  geom_point(aes(color=as.character(AR_Year)),size=0.5)+
  geom_smooth(aes(color=as.character(AR_Year)), se = FALSE)+
